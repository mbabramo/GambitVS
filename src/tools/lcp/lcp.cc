...
void PrintHelp(char *progname)
{
  ...
  std::cerr << "  -v               print version information\n";
  exit(1);
}
...
//  while ((c = getopt_long(argc, argv, "d:DvhqSPe:r:", long_options, &long_opt_index)) != -1) 
  for(int i=1;i<argc;i++)
  {
    ...
    case 'd':
      useFloat = true;
      numDecimals = atoi(argv[i+1]);
      break;
    case 'D':
      printDetail = true;
      break;
    case 'e':
      stopAfter = atoi(argv[i + 1]);
      break;
    case 'h':
      PrintHelp(argv[0]);
      break;
    ...
    case '?':
      if (isprint(argv[i][1])) {
	std::cerr << argv[0] << ": Unknown option `-" << ((char)argv[i][1]) << "'.\n";
      }
      else {
	std::cerr << argv[0] << ": Unknown option character `\\x" << argv[i][1] << "`.\n";
      }
      return 1;
    default:
      abort();
